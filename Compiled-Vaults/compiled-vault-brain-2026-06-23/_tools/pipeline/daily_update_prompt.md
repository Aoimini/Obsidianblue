...
## Target day
The day being summarized is stated at the top of `local_delta_<date>.md` ("target day <date>") —
usually **yesterday**. Use that `<date>` for all output filenames below.

## Inputs (read these first)
- Delta files in `~/.local/share/vaultbrain/inbox/` for the target `<date>`:
  `local_delta_<date>.md` (changed Obsidian notes — **including that day's daily note and any
  voice-diary text the user wrote in its `## 1. MEMO`**), `chrome_<date>.md` (that day's Google
  searches + visited pages), `screentime_<date>.md` (SNS usage, if available), and if present
  `notion_<date>.md` (recent Notion rows).
...
6. **Write a daily digest** at `Reports/daily-digests/<date>.md` (front-matter `type: digest`,
   `date: <date>`). It MUST contain a memo-block in EXACTLY this shape (a deterministic script
   lifts the region between the markers and inserts it into that day's Obsidian `## 1. MEMO`):

   ```
   <!-- memo-block:start -->
   ### 🤖 自動まとめ（<date>）
   **行動ハイライト**
   - …（その日の日記/音声日記・Chrome訪問・Notionから、実際にした事を3-6点。簡潔に）
   **検索テーマ**
   - …（その日のGoogle検索を3-6テーマに束ねる。生URLは貼らない。例:「代アニ エンタメ事業部」→入社先リサーチ）
   **気付き**
   ...
   **PC・SNS利用**
   ...
   should read cleanly as plain Obsidian markdown (this goes into the user's diary).
