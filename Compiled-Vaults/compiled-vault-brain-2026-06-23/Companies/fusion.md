---
type: company
name: "FUSION"
aliases:
  - "FUSION株式会社"
tags:
  - company
  - career-target
  - active-selection
  - advertising
source_status: source-backed
sensitivity: personal
last_verified: 2026-06-23
---

# FUSION

## Summary
[[People/endo-aoi]] の**進行中の転職検討/選考先**。広告運用領域の企業。転職7軸の「⑥広告運用」の具体候補（[[Sources/src-local-tenshoku-md]] に "ex.FUSION" の記載）。

## Relationship To User
- 関係: 進行中（6/9「FUSION準備」「FUSION返信」のタスクがカレンダーに存在）
- Confidence: medium-high
- 位置づけ: 広告運用は「独立時の知識がつく」が「純粋に好きではない」と本人評（GOOD/MOREの整理あり）。

## Related
- [[People/endo-aoi]]
- [[Decisions/career-change-2026]]

## Activity Log
- **年収リサーチ（06-26）**: 「fusion 中途年収」「希望年収 低く言ってしまった」「転職希望年収 聞かれたら」をChrome検索。社員「馬場」「紅林」も検索（面接前のリサーチか）。年収交渉に関する不安・準備が見られる。note.com「新卒座談会 FUSIONの成長環境」も閲覧。
  Source: [[Sources/src-auto-2026-06-26]]

## Provenance
- [[Sources/src-gcal-2026]] — FUSION準備/返信タスク
- [[Sources/src-local-tenshoku-md]] — 広告運用軸 ex.FUSION
